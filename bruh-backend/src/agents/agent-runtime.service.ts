...
export class AgentRuntimeService {
    ...
    async run(input: {
        ...
        try {
            ...
            console.log("Wallet loaded:", wallet.id);
            ...
            const results: RunAgentResult["runs"] = [];
        ...
        return {
            news: {
                async search(input) {
                    return {
                        ...
                        summary:
                            `No external news adapter was enabled. Analysing available market context for "${input.query}".`,
            ...
            historical: {
                async analyze(input) {
                    return {
                        provider:
                            "backend-historical-bridge",

                        summary:
                            `No historical comparator was enabled for "${input.question}".`,
            ...
            onchain: {
                async analyze(input) {
                    return {
                        provider:
                            "backend-onchain-bridge",

                        summary:
                            `No onchain adapter was enabled for "${input.question}".`,
    ...
    private async insertInitialRun(
        input: {
            runId: string;
    ...
    private async persistRuntimeResult(
        ...
        if (error) {
            throw new Error(
                `Failed to persist agent run: ${error.message}`,
            );
        }
    }
