...
export class AgentsService {
  ...
  private async runAgentCycle(wallet: any) {
    const strategies =
      wallet.strategy === 'both' ? ['newshound', 'actuary'] : [wallet.strategy];
  ...
  private async processMarket(
    ...
    const usdcAmount = Math.floor(
      wallet.max_position_usdc * edge * wallet.kelly_fraction,
    );
    console.log(
      `[${strategy}] ${isYes ? 'BUY_YES' : 'BUY_NO'} ${usdcAmount / 1e6} USDC on "${question}"`,
    );
