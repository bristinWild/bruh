...
export class ExecutionService {
    ...
    async executePlan(
        input: ExecuteStoredPlanInput,
    ): Promise<ExecutionReceipt> {
        const existing =
            await this.getReceiptByPlanId(
                input.plan.id,
            );
    ...
    async getReceiptByPlanId(
        ...
        if (error) {
            throw new Error(
                `Failed to read execution receipt: ${error.message}`,
            );
        }
    ...
    async listWalletReceipts(
        ...
        if (error) {
            throw new Error(
                `Failed to list execution receipts: ${error.message}`,
            );
        }
    ...
    private async getAgentWallet(
        ...
        const {
            ...
            await this.supabase.db
                .from("agent_wallets")
                .select(
                    "id, circle_wallet_id, wallet_address, address",
                )
                .eq("id", walletId)
                .single();
    ...
    private async persistReceipt(input: {
        ...
        if (error || !data) {
            throw new Error(
                `Failed to persist execution receipt: ${error?.message ??
    ...
    private async persistConfirmedTrade(
        input: {
            plan: ExecutionPlan;
...
function resolveUsdcAddress():
    ...
    if (
        ...
        throw new Error(
            "ARC_USDC_ADDRESS must be a valid EVM address.",
        );
    }
